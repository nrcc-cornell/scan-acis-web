...
const DisplayHeatIndexChart = ({data, stnName}) => {
        let max = 0;
        let min = 0;
        if (data.length > 0) {
          for (let i = 0; i < data.length; i++) {
            const heatIndex = data[i][1];
            if (heatIndex < min) min = heatIndex;
            if (max < heatIndex) max = heatIndex;
          }
        }
...
DisplayHeatIndexChart.propTypes = {
  data: PropTypes.object.isRequired,
  stnName: PropTypes.string.isRequired,
};

export default DisplayHeatIndexChart;
